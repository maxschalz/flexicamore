...
/*
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichmentTest::SetUp() {
  cyclus::PyStart();
  cyclus::Env::SetNucDataPath();

  fake_sim = new cyclus::MockSim(1);
  flex_enrich_facility = new SharedSwuEnrichment(fake_sim->context());

  InitParameters();
  SetUpSharedSwuEnrichment();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichmentTest::TearDown() {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichmentTest::InitParameters() {
  ...
  du_recipe = "DURecipe";
  fake_sim->AddRecipe("NURecipe", shared_swu_test::NaturalU());
  fake_sim->AddRecipe("LEURecipe", shared_swu_test::LowEnrichedU());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichmentTest::SetUpSharedSwuEnrichment() {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, Print) {
  EXPECT_NO_THROW(std::string s = flex_enrich_facility->str());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, Tick) {
  EXPECT_NO_THROW(flex_enrich_facility->Tick());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, Tock) {
  EXPECT_NO_THROW(flex_enrich_facility->Tock());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, DISABLED_GetMatlRequests) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, CheckFeedQty) {
  for (int i = 0; i < feed_commods.size(); ++i) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, CheckFeedPrefs) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, AddFeedMaterial) {
  cyclus::Material::Ptr correct_mat = cyclus::Material::CreateUntracked(
      inv_size, shared_swu_test::NaturalU());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, Enrich) {
  ...
  Material::Ptr product = Material::CreateUntracked(product_qty,
                                                    shared_swu_test::HighlyEnrichedU());

  EXPECT_THROW(DoEnrich(product, product_qty), cyclus::ValueError);

  DoAddFeedMat(Material::CreateUntracked(inv_size, shared_swu_test::NaturalU()),
               feed_commods[0]);
  ...
  DoAddFeedMat(Material::CreateUntracked(inv_size,
shared_swu_test::LowEnrichedU()), feed_commods[1]);
  ASSERT_NO_THROW(DoEnrich(product, product_qty));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, GetMatlBids) {
  ...
  Material::Ptr product = Material::CreateUntracked(1,
shared_swu_test::WeapongradeU()); Material::Ptr tails =
Material::CreateUntracked(1, shared_swu_test::DepletedU()); req_prod =
cyclus::Request<Material>::Create(product, flex_enrich_facility,
                                               product_commod);
  ...
  DoAddFeedMat(cyclus::Material::CreateUntracked(
        inv_size, shared_swu_test::NaturalU()), feed_commods[0]);
  bids = flex_enrich_facility->GetMatlBids(out_requests);
  EXPECT_EQ(1, bids.size());

  DoAddFeedMat(cyclus::Material::CreateUntracked(
        inv_size, shared_swu_test::LowEnrichedU()), feed_commods[1]);
  bids = flex_enrich_facility->GetMatlBids(out_requests);
  EXPECT_EQ(1, bids.size());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, DISABLED_BidPrefs) {
  // Test disabled, see
https://git.rwth-aachen.de/nvd/fuel-cycle/flexicamore/-/issues/4
  ...
  int simdur = 1;
  cyclus::MockSim sim(cyclus::AgentSpec(":flexicamore:SharedSwuEnrichment"),
                      config, simdur);
  sim.AddRecipe(nu_recipe, shared_swu_test::NaturalU());
  sim.AddRecipe(leu_recipe, shared_swu_test::LowEnrichedU());
  ...
  cyclus::CompMap expected = shared_swu_test::LowEnrichedU()->mass();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, DISABLED_NoBidPrefs) {
  // Test disabled, see
https://git.rwth-aachen.de/nvd/fuel-cycle/flexicamore/-/issues/4
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, CheckSWUConstraint) {
  ...
  cyclus::MockSim sim(cyclus::AgentSpec(":flexicamore:SharedSwuEnrichment"),
                      config, simdur);
  sim.AddRecipe(nu_recipe, shared_swu_test::NaturalU());
  sim.AddRecipe(wgu_recipe, shared_swu_test::WeapongradeU());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, CheckMultipleSWUConstraints) {
  // Ensure that requests exceeding the SWU constraint are fulfilled only up to
...
<product_commod>"+product_commod+"</product_commod> " "
<tails_commod>depleted_U</tails_commod> " "   <tails_assay>0.003</tails_assay> "
    "   <max_feed_inventory>1000</max_feed_inventory> "
    "   <order_prefs>0</order_prefs>"
    "   <swu_capacity_times><val>0</val></swu_capacity_times> "
    "   <swu_capacity_vals><val>10</val></swu_capacity_vals> ";
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, FeedConstraint) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, RequestSim) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, TailsTrade) {
  std::string config =
    "   <feed_commods>"
    "     <val>"+feed_commods[0]+"</val> "
    "     <val>"+feed_commods[1]+"</val> "
  ...
  cyclus::MockSim sim(cyclus::AgentSpec(":flexicamore:SharedSwuEnrichment"),
                      config, simdur);
  sim.AddRecipe(nu_recipe, shared_swu_test::NaturalU());
  sim.AddRecipe(du_recipe, shared_swu_test::DepletedU());
  sim.AddRecipe(wgu_recipe, shared_swu_test::WeapongradeU());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TEST_F(SharedSwuEnrichmentTest, ValidRequest) {
  ...
  Material::Ptr naturalU = Material::CreateUntracked(1,
shared_swu_test::NaturalU()); Material::Ptr depletedU =
Material::CreateUntracked(1, comp_depletedU); Material::Ptr no_U238 =
Material::CreateUntracked(1, comp_no_U238); Material::Ptr weapongradeU =
Material::CreateUntracked(1, shared_swu_test::WeapongradeU());
