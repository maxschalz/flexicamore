...
SharedSwuEnrichment::SharedSwuEnrichment(cyclus::Context* ctx)
    : cyclus::Facility(ctx),
      ...
      swu_capacity_times(std::vector<int>({-1})),
      swu_capacity_vals(std::vector<double>({-1})),
      flexible_swu(FlexibleInput<double>()),
      intra_timestep_feed(std::vector<double>({})),
      intra_timestep_swu(0.) {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
SharedSwuEnrichment::~SharedSwuEnrichment() {}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
std::string SharedSwuEnrichment::str() {
  std::stringstream ss;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichment::EnterNotify() {
  ...
  if (swu_capacity_vals[0] != -1) {
    if (swu_capacity_times[0] == -1) {
      ...
      flexible_swu =
          FlexibleInput<double>(this, swu_capacity_vals, swu_capacity_times);
    }
  }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichment::AddFeedMat_(cyclus::Material::Ptr mat,
                                      std::string commodity) {
  std::vector<std::string>::iterator commod_it =
      std::find(feed_commods.begin(), feed_commods.end(), commodity);
  if (commod_it == feed_commods.end()) {
    std::string msg =
        "tried to add a material to the feed inventory which is "
  ...
  LOG(cyclus::LEV_INFO5, "FlxEnr")
      << prototype() << " is initially holding "
      << feed_inv[push_idx].quantity() << " of feed (commodity: " << commodity
  ...
  LOG(cyclus::LEV_INFO5, "FlxEnr")
      << prototype() << " added " << mat->quantity() << " of feed commodity '"
      << commodity << "' to its inventory no. " << push_idx
      << " which is now holding " << feed_inv[push_idx].quantity();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichment::FeedIdxByPreference_() {
  ...
  std::stable_sort(feed_idx_by_pref.begin(), feed_idx_by_pref.end(),
                   [&](int i, int j) {
                     return feed_commod_prefs[i] > feed_commod_prefs[j];
                   });
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichment::Tick() {
  ...
  // Only the enrichment facility that has a valid, non-default definition of
  // `swu_capacity_vals` (from the input file) is allowed to update
  // `swu_capacity`.
  if (swu_capacity_vals[0] != -1) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichment::Tock() {
  using cyclus::toolkit::RecordTimeSeries;

  LOG(cyclus::LEV_INFO4, "FlxEnr")
      << prototype() << " used " << intra_timestep_swu << " SWU";
  RecordTimeSeries<cyclus::toolkit::ENRICH_SWU>(this, intra_timestep_swu);

  for (int i = 0; i < feed_commods.size(); ++i) {
    LOG(cyclus::LEV_INFO4, "FlxEnr")
        << prototype() << " used " << intra_timestep_feed[i] << " feed"
    ...
    // commodity.
    RecordTimeSeries<cyclus::toolkit::ENRICH_FEED>(this, intra_timestep_feed[i],
                                                   feed_commods[i]);
...
SharedSwuEnrichment::GetMatlRequests() {
  using cyclus::Material;
  ...
  for (int i = 0; i < feed_inv.size(); ++i) {
    double amount =
        std::min(max_feed_inventory, std::max(0., feed_inv[i].space()));
...
SharedSwuEnrichment::GetMatlBids(
  ...
  RecordTimeSeries<double>("supply" + tails_commod, this, tails_inv.quantity());
  ...
  if ((out_requests.count(tails_commod) > 0) && (tails_inv.quantity() > 0)) {
    ...
    LOG(cyclus::LEV_INFO5, "FlxEnr")
        << prototype() << " adding tails capacity constraint"
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
double SharedSwuEnrichment::FeedAssay_(int feed_idx_) {
  using cyclus::Material;
  ...
  cyclus::Material::Ptr feed_mat =
      feed_inv[feed_idx_].Pop(pop_qty, cyclus::eps_rsrc());
  feed_inv[feed_idx_].Push(feed_mat);
  return cyclus::toolkit::UraniumAssayMass(feed_mat);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
cyclus::Material::Ptr SharedSwuEnrichment::Offer_(cyclus::Material::Ptr mat) {
  ...
  return cyclus::Material::CreateUntracked(
      mat->quantity() / uranium_frac, cyclus::Composition::CreateFromAtom(cm));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool SharedSwuEnrichment::ValidReq_(const cyclus::Material::Ptr req_mat) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// TODO create separate file with other functions where this is included and
// which is shared between FlexibleEnrichment and SharedSwuEnrichment.
//
// `SharedSwuSortBids` must not be a non-static member function of
// `SharedSwuEnrichment` because of its usage in `std::sort` and it has to be
// defined *before* `AdjustMatlPrefs` as it is not declared.
bool SharedSwuSortBids(cyclus::Bid<cyclus::Material>* i,
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichment::AdjustMatlPrefs(
  ...
  for (reqit = prefs.begin(); reqit != prefs.end(); ++reqit) {
    ...
    std::sort(bids_vector.begin(), bids_vector.end(), SharedSwuSortBids);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichment::GetMatlTrades(
  ...
  for (it = trades.begin(); it != trades.end(); it++) {
    ...
    if (commod_type == tails_commod) {
      LOG(cyclus::LEV_INFO5, "FlxEnr")
          << prototype() << " just received an order for " << qty << " of "
          << tails_commod;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
cyclus::Material::Ptr SharedSwuEnrichment::Enrich_(cyclus::Material::Ptr mat,
                                                   double request_qty) {
  ...
  for (int feed_idx : feed_idx_by_pref) {
    ...
    LOG(cyclus::LEV_DEBUG5, "FlxEnr")
        << "Considering feed commod " << feed_commods[feed_idx];
    double feed_assay = FeedAssay_(feed_idx);
    ...
    cyclus::Material::Ptr feed_mat =
        feed_inv[feed_idx].Pop(feed_inv_qty, cyclus::eps_rsrc());
    ...
    if (feed_inv_qty < uranium_required) {
      LOG(cyclus::LEV_DEBUG5, "FlxEnr")
          << "Not enough " << feed_commods[feed_idx] << " present.\n";
      continue;
    }
    LOG(cyclus::LEV_DEBUG5, "FlxEnr")
        << "using feed commod " << feed_commods[feed_idx];
  ...
  if (feed_used_idx == -1) {
    for (int feed_idx : feed_idx_by_pref) {
      if (feed_inv[feed_idx].quantity() > cyclus::eps_rsrc()) {
        LOG(cyclus::LEV_DEBUG5, "FlxEnr")
            << "fallback to " << feed_commods[feed_idx];
        feed_used_idx = feed_idx;
        break;
      }
    }
  ...
  cyclus::Material::Ptr feed_mat =
      feed_inv[feed_used_idx].Pop(pop_qty, cyclus::eps_rsrc());
  ...
  cyclus::Material::Ptr response =
      pop_mat->ExtractComp(request_qty, product_comp);
  ...
  LOG(cyclus::LEV_INFO5, "FlxEnr")
      << "   * Feed Commod.: " << feed_commods[feed_used_idx];
  ...
  LOG(cyclus::LEV_INFO5, "FlxEnr")
      << "   * Tails Qty: " << cyclus::toolkit::TailsQty(request_qty, assays);
  LOG(cyclus::LEV_INFO5, "FlxEnr")
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichment::AcceptMatlTrades(
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichment::AddMat_(cyclus::Material::Ptr mat,
                                  std::string commodity) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichment::RecordEnrichment_(double feed_qty, double swu,
                                            std::string feed_commod) {
  LOG(cyclus::LEV_DEBUG1, "FlxEnr")
  ...
  ctx->NewDatum("SharedSwuEnrichments")
      ->AddVal("AgentId", id())
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SharedSwuEnrichment::RecordPosition() {
...
// archetype to fail.
extern "C" cyclus::Agent* ConstructSharedSwuEnrichment(cyclus::Context* ctx) {
  return new SharedSwuEnrichment(ctx);
}
